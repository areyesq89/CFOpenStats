...
according to the official website, that consist of three workouts 
...
series of the following exercises with a time cap of 15 minutes,
...
An impressive number of `r nrow(allLeaderboardClean)` participants submitted a score 
to the CrossFit Open website, of which `r round(100*sum(allLeaderboardClean$workoutType == "Rx")/nrow(allLeaderboardClean))`% participated in the Rx category, `r round(100*sum(allLeaderboardClean$workoutType == "scaled")/nrow(allLeaderboardClean))`% 
participated in the scaled category and `r round(100*sum(allLeaderboardClean$workoutType == "foundations")/nrow(allLeaderboardClean))`% participated in the foundations 
category. 
...
for each scaling version for both male and female divisions. Across all 
scaling versions, only `r round(100*sum(allLeaderboardClean$timeInSecs < 10*60)/nrow(allLeaderboardClean))`%
completed the workout in less than 10 minutes and `r round(100*sum(allLeaderboardClean$reps < 180)/nrow(allLeaderboardClean))`% could not complete all the repetitions
within the 15 minute mark. A very though workout!

## How many seconds away am you from quater-final scores? {-}

Athletes that rank among the top 25% in the Rx category classify to the quarterfinals. 
Considering the data from 24.1. The table \@ref(fig:global-dist) shows 
the completion times you'd have need to do in 24.1 in order to be 
within the fastest 25% in your age category. 

```{r thetable, results='asis'}

allLeaderboardClean |>
  dplyr::filter( workout == "24.1", workoutType == "Rx" ) |>
  dplyr::group_by( division, ageGroup ) |>
  dplyr::summarize( 
    `completion time`=seconds_to_period(quantile(as.numeric(timeInSecs), 0.25))) |>
  dplyr::rename( `Age group`=ageGroup ) |>
  knitr::kable(caption = "Time cap of the 25% fastest in 24.1.") |>
...
the workout in 13:43, meaning that I'm more than 2 minutes away from being
a quarter final athlete!

## Comparing scores between CF boxes {-}

I extracted the boxes with the best average scores worldwide, and plotted
the completion times together with the scores of the CF boxes in Basel, where 
I live and workout (Figure \@ref(fig:box) ). 
...
completion times of my box (CF HelvetiX) is very similar to the world wide 
distributions for the male division. But for the female division, 
females in CF HelvetiX are much more stronger than the average and closer to 
best boxes around the globe.

## Conclusion {-}

If you are betting for someone in Basel to classify to the quarter finals,
bet for a CF HelvetiX female athlete!